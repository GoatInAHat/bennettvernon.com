 ...
 */
export interface CellCensusConfig {
  /** World-space disc to scan. */
  centerX: number;
  centerY: number;
  radius: number;
  /** Classification grid: world origin, cell size, and dimensions. */
  gridMinX: number;
  gridMinY: number;
  gridCell: number;
  gridCols: number;
  gridRows: number;
  /** Per grid cell: census cell index, or -1 for unclassified. */
  cells: Int32Array;
  /** Bump when `cells` content changes; gates the GPU re-upload. */
  version: number;
  cellCount: number;
  /** Candidate particle indices collected per cell. */
  samplesPerCell: number;
  /** Reservoir size for in-disc particles outside every cell. */
...
export interface IEngine {
   ...
   * result (typically 1–2 frames old) is returned; on CPU it is computed
