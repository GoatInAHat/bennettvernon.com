...
export class ModuleRegistry {
  ...
  /** Last offsets uploaded per module, to skip re-writing unmoved arrays. */
  ...
  private writeModuleUniform(
    ...
    if (hasArrayUpdate && this.moduleArrayState[name]) {
      ...
      const prevOffsets = this.moduleArrayOffsets[name] ?? {};
      let currentOffset = 0;
      ...
      // Only unchanged arrays whose offset also stayed put can skip the
      // upload; if the combined buffer must grow, everything is rewritten
      // into the fresh buffer.
      const capacity =
        this.resources.getCombinedArrayStorageCapacity(name) / 4;
      const mustGrow = currentOffset > capacity;
      for (const arrayKey of arrayKeys) {
        if (!arrayDataMap[arrayKey]) {
          if (mustGrow || prevOffsets[arrayKey] !== arrayOffsets[arrayKey]) {
            arrayDataMap[arrayKey] = this.moduleArrayState[name][arrayKey];
          }
        }
      }
      this.moduleArrayOffsets[name] = arrayOffsets;
    }
