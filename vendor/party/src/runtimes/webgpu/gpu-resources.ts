...
export class GPUResources {
  ...
  /** Current byte capacity of a module's combined array buffer (0 if none). */
  getCombinedArrayStorageCapacity(moduleName: string): number {
    return this.combinedArrayStorageBuffers.get(moduleName)?.size ?? 0;
  }
